...
class RxBleClientTestCompat extends Specification {
    ...
    MockRxBleScannerCompat bleScannerCompatSpy = Spy MockRxBleScannerCompat
    ...
    def setup() {
        ...
        objectUnderTest = new RxBleClientImplCompat(
    ...
    def "should start BLE scan if subscriber subscribes to the scan observable"() {
        ...
        then:
        1 * bleScannerCompatSpy.startScanCompat(_,_,_)
    }
    ...
    def "should not start scan until observable is subscribed"() {
        ...
        then:
        0 * bleScannerCompatSpy.startScanCompat(_,_,_)
    }
    ...
    def "should stop scan after subscriber is unsubscribed from scan observable"() {
        ...
        then:
        1 * bleScannerCompatSpy.stopScanCompat(_)
    }

    def "should stop and unsubscribe in case of scan throws exception"() {
        given:
        TestSubscriber testSubscriber = new TestSubscriber<>()
        bleScannerCompatSpy.startScanCompat(_,_,_) >>{throw new NullPointerException()}
    ...
    def "should not stop scan if not all subscribers are unsubscribed"() {
        ...
        then:
        0 * bleScannerCompatSpy.stopScanCompat(_)
    }
    ...
    def "should emit BleScanException if bluetooth scan failed to start"() {
        given:
        TestSubscriber firstSubscriber = new TestSubscriber<>()
        bleScannerCompatSpy.startScanCompat(_,_,_)>> { throw new IllegalArgumentException("Scan callback and settings cannot be null")}
